    ...
    for (int imp = 0; imp < 4; ++imp)
    {
        ...
        switch (imp)
        {
            case 3:
            {
                ...
                cout << endl
                     << "Using SumFac Collection Implementation:" << endl;
            }
            break;
            case 2:
            {
                ...
                cout << endl
                     << "Using StdMat Collection Implementation:" << endl;
            }
            break;
            case 1:
            {
        ...
        for (int N = 2; N < 7; ++N)
        {
            ...
            Exp = MemoryManager<MultiRegions::ExpList3D>::AllocateSharedPtr(
                vSession, graph3D);
            ...
            for (int i = 0; i < Ntest; ++i)
            {
                ...
                {
                    Exp->GetExp(j)
                        ->BwdTrans(coeffs + Exp->GetCoeff_Offset(j),
                                   tmp = phys1 + Exp->GetPhys_Offset(j));
                }
            }
            t.Stop();
            NekDouble orig = t.TimePerTest(Ntest);
            cout << "N: " << std::setw(2) << N << " ExpList: " << std::setw(11)
                 << orig;
            t.Start();
        ...
        for (int N = 2; N < 7; ++N)
        {
            ...
            for (int i = 0; i < Ntest; ++i)
            {
                ...
                for (int j = 0; j < nelmt; ++j)
                {
                    Exp->GetExp(j)->IProductWRTBase(
                        input + Exp->GetPhys_Offset(j),
        ...
        for (int N = 2; N < 7; ++N)
        {
            ...
            for (int i = 0; i < Ntest; ++i)
            {
                ...
                {
                    Exp->GetExp(j)
                        ->PhysDeriv(input + Exp->GetPhys_Offset(j),
        ...
        for (int N = 2; N < 7; ++N)
        {
            ...
            const int nc = Exp->GetNcoeffs();
            Array<OneD, NekDouble> xc(nq), yc(nq), zc(nq), tmp, tmp1;
            Array<OneD, NekDouble> input1(nq), input2(nq), input3(nq),
                output1(nc), output2(nc);
            ...
            for (int i = 0; i < Ntest; ++i)
            {
                ...
                for (int j = 0; j < nelmt; ++j)
                {
                    Exp->GetExp(j)->IProductWRTDerivBase(
                        0,
                        input1 + Exp->GetPhys_Offset(j),
                        tmp = output1 + Exp->GetCoeff_Offset(j));
                    Exp->GetExp(j)->IProductWRTDerivBase(
                        1,
                        input2 + Exp->GetPhys_Offset(j),
