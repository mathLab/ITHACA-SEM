	...
	{
	    fprintf(stderr,
		    "Usage: ./Gentien meshfile fieldFile Option Nprof L Npoint xo_start xo_end\n");
	    fprintf(stderr,
		    "Extract the data along the normal of the wall ");
	    exit(1);
	}
	...
	cout << "Reading the mesh and calculating the data ... please wait" << endl;
	...
	// Get L parameter from session file.
	ASSERTL0(vSession->DefinesParameter("L"),
		"Compressible flow sessions must define a uInf parameter.");
	vSession->LoadParameter("L", m_L, 0.1);
	
	// Get Mach parameter from session file.
	ASSERTL0(vSession->DefinesParameter("Mach"),
		"Compressible flow sessions must define a uInf parameter.");
	vSession->LoadParameter("Mach", m_Mo, 0.1);
	
	// Get Mach parameter from session file.
	ASSERTL0(vSession->DefinesParameter("TInf"),
		"Compressible flow sessions must define a uInf parameter.");
	vSession->LoadParameter("TInf", m_TInf, 0.1);
	
	// Get Reynolds parameter from session file.
	ASSERTL0(vSession->DefinesParameter("Re"),
		"Compressible flow sessions must define a uInf parameter.");
	vSession->LoadParameter("Re", m_Ro, 0.1);
	...
	**/
	...
	Array<OneD, Array<OneD, Array<OneD, Array<OneD, NekDouble> > > > DDvelocity(nDimensions);
	Array<OneD, Array<OneD, NekDouble> > DTemperature(nDimensions);
	Array<OneD, Array<OneD, Array<OneD, NekDouble> > > DDTemperature(nDimensions);
	

	for ( i = 0; i < nDimensions; ++i)
	{
	    DDvelocity[i] = Array<OneD, Array<OneD, Array<OneD, NekDouble> > >(nDimensions);
	    DTemperature[i] = Array<OneD, NekDouble>(nSolutionPts, 0.0);
	    DDTemperature[i] = Array<OneD, Array<OneD, NekDouble> >(nDimensions);
	    
	    for (j = 0; j < nDimensions; ++j)
	    {
		DDvelocity[i][j]      = Array<OneD, Array<OneD, NekDouble> >(nDimensions);
		DDTemperature[i][j] = Array<OneD, NekDouble>(nSolutionPts, 0.0);
	    
			for (int k = 0; k < nDimensions; ++k)
			{
			DDvelocity[i][j][k]     = Array<OneD, NekDouble>(nSolutionPts, 0.0);
			}
		}
	}
	...
	for (i = 0; i < nDimensions; ++i)
	{
	    ...
	    for ( j = 0; j < nDimensions; ++j)
	    {
				...
				pFields[i]->GetExp(n)->PhysDeriv(
				j, DTemperature[i] + phys_offset,
				auxArray = DDTemperature[i][j] + phys_offset);
				}
			}
	}
	...
	for (i = 0; i < nDimensions; ++i)
	{
	    for ( j = 0; j < nDimensions; ++j)
	    {
			for (int k = 0; k < nDimensions; ++k)
			{
				...
				pFields[i]->GetExp(n)->PhysDeriv(
				k, Dvelocity[i][j] + phys_offset,
				auxArray = DDvelocity[i][j][k] + phys_offset);
				}
			}
	    }
	}
...
// 			Vmath::Vdiv(nbc,lift_v,1,CoeffAero,1, lift_v,1);
    ...
    // Print the surface coordinates in a .txt file
	ofstream outfile; 
    ...
    //------------------------------------------------------------------------------------------------------------------------
    // We start now the extraction,
    // -----------------------------------------------------------------------------------------------------------------------
   
	 
    
      NekDouble Npoint = atoi(argv[6]);
      NekDouble Nprof= atoi(argv[4]);
      NekDouble xo_start= atof(argv[7]);
      NekDouble xo_end= atof(argv[8]);
      NekDouble x0=  0.0;
      NekDouble y0=  0.0;
      NekDouble z0=  0.0;
      NekDouble Re=0.0;
      ...
      NekDouble delta=0.0;
	  NekDouble velo=0.0;
	  NekDouble intermediaire=0.0;
	  NekDouble dy= atof(argv[5])/(Npoint>1 ? (Npoint-1):1);  
	  NekDouble dx= (xo_end-xo_start)/(Nprof>1 ? (Nprof-1):1); 
	  NekDouble mu_e=0.0;
      ...
      int ExpId;  
		...
		if (atof(argv[3])==1)
		{	
		file.open("resultat1.pro");
		}
		else
		{
		file.open("resultat2.pro");
		}			
      
      
		file << "Extraction of the data along the normal" << endl;
		file <<  scientific << setprecision(9) << m_uInf << "   " << m_Mo << "   " << m_Ro << "   " << m_TInf << "   " << m_L << "   " << 0.0 << "   " << endl;
		file <<   int(Nprof) << endl;
		...
		{	
			...
			// Recharch of the delta and data for y=delta
						...
						if (atof(argv[3])==1)
						{
						  ...
						  {	
									gloCoord[1] = y0 + j*0.010/(interation-1);
							
									ExpId =  Exp[0]->GetExpIndex(gloCoord,NekConstants::kGeomFactorsTol);
									...
									{
									Array<OneD, NekDouble> phys(velocity[0] + Exp[0]->GetPhys_Offset(ExpId));
									velo=Exp[0]->GetExp(ExpId)->PhysEvaluate(gloCoord,phys);
									}
								
								if (velo>=0.999*m_uInf) 
								{
									indice=j;
							...
							delta=indice*0.010/(interation-1);
						}
						
						else
						{	
							 
							delta=4.91*sqrt((x0*m_mu)/(m_rhoInf*m_uInf));
						}
							...
							//cout << xo_start << "    " << xo_end << endl;
							cout << "Calcul in progress" << "   " << i 
							<< "/" << Nprof << "    "  << gloCoord[0] << "   "  << gloCoord[1] << endl;
							...
							{
							Array<OneD, NekDouble> phys2(Exp[0]->GetPhys() + Exp[0]->GetPhys_Offset(ExpId));
							Re=Exp[0]->GetExp(ExpId)->PhysEvaluate(gloCoord,phys2);
							
							Array<OneD, NekDouble> phys3(temperature + Exp[0]->GetPhys_Offset(ExpId));
							Te=Exp[0]->GetExp(ExpId)->PhysEvaluate(gloCoord,phys3);
							
							Array<OneD, NekDouble> phys4(pressure + Exp[0]->GetPhys_Offset(ExpId));
							Pe=Exp[0]->GetExp(ExpId)->PhysEvaluate(gloCoord,phys4);
							
							Array<OneD, NekDouble> phys5(Dpressure[0] + Exp[0]->GetPhys_Offset(ExpId));
							Ps=Exp[0]->GetExp(ExpId)->PhysEvaluate(gloCoord,phys5);
							
							Array<OneD, NekDouble> phys6(mu + Exp[0]->GetPhys_Offset(ExpId));
							mu_e=Exp[0]->GetExp(ExpId)->PhysEvaluate(gloCoord,phys6);
							
							Pe=(Pe)/(Re*m_uInf*m_uInf);
							Ps=Ps*(m_L/(Re*m_uInf*m_uInf));
							Re=(Re*m_uInf*delta)/mu_e;	
							}
							
							if (isfinite(y0/delta)==1)
							{	
								
			file << i << "  " << int(Npoint) << "  "<< ((i)/(Nprof-1))  << "  " << (i)/(Nprof-1) << "  " << 1.0 << "  " << 1.0 << endl;
			file << m_uInf << "  " << m_Mo << "  " <<  Re << "  "<< Te << "  " << delta << "  " << Pe << "  " ;
			file << "  " << Ps << endl;
				...
				for (int k = 0; k < Npoint ; k++)
						{						
							...
							intermediaire=gloCoord[1] ;
							file << intermediaire/delta <<"  " ;
							
							Array<OneD, NekDouble> velo_u(velocity[0] + Exp[0]->GetPhys_Offset(ExpId));
							intermediaire=Exp[0]->GetExp(ExpId)->PhysEvaluate(gloCoord,velo_u);
							file << intermediaire/m_uInf <<"  " ;
							
							Array<OneD, NekDouble> Dvelo_u_y(Dvelocity[0][1] + Exp[0]->GetPhys_Offset(ExpId));
							intermediaire=Exp[0]->GetExp(ExpId)->PhysEvaluate(gloCoord,Dvelo_u_y)  ;
							file << intermediaire*(delta/m_uInf) <<"  " ;
											
							Array<OneD, NekDouble> DDvelo_u_y_y(DDvelocity[0][1][1] + Exp[0]->GetPhys_Offset(ExpId));
							intermediaire=Exp[0]->GetExp(ExpId)->PhysEvaluate(gloCoord,DDvelo_u_y_y) ;
							file << intermediaire*(delta*delta/m_uInf) <<"  " ;
							
							Array<OneD, NekDouble> Dvelo_u_s(Dvelocity[0][0] + Exp[0]->GetPhys_Offset(ExpId));
							intermediaire=Exp[0]->GetExp(ExpId)->PhysEvaluate(gloCoord,Dvelo_u_s);
							file << intermediaire*(m_L/m_uInf) <<"  " ;
							
							Array<OneD, NekDouble> DDvelo_u_y_s(DDvelocity[0][1][0] + Exp[0]->GetPhys_Offset(ExpId));
							intermediaire=Exp[0]->GetExp(ExpId)->PhysEvaluate(gloCoord,DDvelo_u_y_s);
							file << intermediaire*(delta*m_L/m_uInf) <<"  " ;
							
							file << 0.0 << "  "  ;
							
							Array<OneD, NekDouble> velo_v(velocity[1] + Exp[0]->GetPhys_Offset(ExpId));
							intermediaire=Exp[0]->GetExp(ExpId)->PhysEvaluate(gloCoord,velo_v) ;
							file << intermediaire*(1.0/m_uInf) <<"  " ;
							...
							Array<OneD, NekDouble> temp(temperature + Exp[0]->GetPhys_Offset(ExpId));
							intermediaire=Exp[0]->GetExp(ExpId)->PhysEvaluate(gloCoord,temp);
							file << intermediaire*(1.0/m_TInf) <<"  " ;
							
							Array<OneD, NekDouble> Dtemp_y(DTemperature[1] + Exp[0]->GetPhys_Offset(ExpId));
							intermediaire=Exp[0]->GetExp(ExpId)->PhysEvaluate(gloCoord,Dtemp_y) ;
							file << intermediaire*(delta/m_TInf) <<"  " ;
							
							Array<OneD, NekDouble> DDtemp_y_y(DDTemperature[1][1] + Exp[0]->GetPhys_Offset(ExpId));
							intermediaire=Exp[0]->GetExp(ExpId)->PhysEvaluate(gloCoord,DDtemp_y_y) ;
							file << intermediaire*(delta*delta/m_TInf) <<"  " ;
							
							Array<OneD, NekDouble> Dtemp_s(DTemperature[0] + Exp[0]->GetPhys_Offset(ExpId));
							intermediaire=Exp[0]->GetExp(ExpId)->PhysEvaluate(gloCoord,Dtemp_s) ;
							file << intermediaire*(m_L/m_TInf) <<"  " ;
							
							Array<OneD, NekDouble> DDtemp_y_s(DDTemperature[1][0] + Exp[0]->GetPhys_Offset(ExpId));
							intermediaire=Exp[0]->GetExp(ExpId)->PhysEvaluate(gloCoord,DDtemp_y_s);
							file << intermediaire*(delta*m_L/m_TInf) <<"  " ;
							
							file << 0.0 << "  " ;  
							
							Array<OneD, NekDouble> Dvelo_v_y(Dvelocity[1][1] + Exp[0]->GetPhys_Offset(ExpId));
							intermediaire=Exp[0]->GetExp(ExpId)->PhysEvaluate(gloCoord,Dvelo_v_y) ;
							file << intermediaire*(delta/m_uInf) <<"  " ;
							
							Array<OneD, NekDouble> DDvelo_v_y_y(DDvelocity[1][1][1] + Exp[0]->GetPhys_Offset(ExpId));
							intermediaire= Exp[0]->GetExp(ExpId)->PhysEvaluate(gloCoord,DDvelo_v_y_y) ;
							file << intermediaire*(delta*delta/m_uInf) << "  " ;
							
							Array<OneD, NekDouble> Dvelo_v_x(Dvelocity[1][0] + Exp[0]->GetPhys_Offset(ExpId));
							intermediaire= Exp[0]->GetExp(ExpId)->PhysEvaluate(gloCoord,Dvelo_v_x) ;
							file << intermediaire*(m_L/m_uInf) << "   " ;
							
							Array<OneD, NekDouble> DDvelo_v_x_y(DDvelocity[1][0][1] + Exp[0]->GetPhys_Offset(ExpId));
							intermediaire= Exp[0]->GetExp(ExpId)->PhysEvaluate(gloCoord,Dvelo_v_x) ;
							file << intermediaire*(m_L*delta/m_uInf) << endl ;
