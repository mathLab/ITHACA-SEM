...
\end{lstlisting}
Also, there are other two optional properties in the \inltt{SolverInfo} section:
\begin{lstlisting}[style=XMLStyle]
<I PROPERTY="MappingImplicitPressure" VALUE="TRUE"/> <!-- Default = FALSE -->
<I PROPERTY="MappingImplicitViscous" VALUE="TRUE"/>  <!-- Default = FALSE -->
\end{lstlisting}
these options determine if the pressure and viscous terms resulting from the
coordinate transformation are treated implicitly using an iterative procedure.
 By default, all mapping terms are treated explicitly.
...
(positive Jacobian). By default, any component of \inltt{COORDS} that
