...
#                 self.writeln()
    ...
    def output_top_boilerplate(self):
        ...
        #counting the number of gating and concentration variables and outputting them
        gate_vars = 0
        concentration_vars = 0
        for var in self.state_vars:
            if str(var).find('membrane__V') != -1:
                continue
            elif str(var).find('gate') != -1:
                gate_vars += 1
                #self.writeln(var, ' gating variable')
            elif str(var).find('gate') == -1:
                concentration_vars += 1
                #self.writeln(var, ' concentration variable')
        
        #writing the gating and concentration variables
        for i in range(1,gate_vars+1):
            self.writeln('m_gates.push_back(',i,');')
        for i in range(gate_vars+1,gate_vars+concentration_vars+1):
            self.writeln('m_concentrations.push_back(',i,');')    
        self.close_block()
...
#            self.writeln('EXCEPT_IF_NOT(!std::isnan(', self.code_name(assigned_var), '));')
    ...
    def output_evaluate_y_derivatives(self, method_name='EvaluateYDerivatives'):
        ...
        self.writeln(' const NekDouble time)', indent_offset=13)
        self.open_block()
        ...
        for i, var in enumerate(self.state_vars):
            ...
            if var_name.find('gate') != -1:
                ...
                self.writeln('outarray[', i ,'][i] = ' , after_underscore , '_inf;')
                self.writeln('m_gates_tau[',i-1,'][i] = ' , after_underscore , '_tau;')
            else:    
                self.writeln(self.vector_index('outarray', i),'[i]',  self.EQ_ASSIGN, var_name, self.STMT_END)
        self.close_block()
        self.close_block()
    ...
    def output_derivative_calculations(self, state_vars, assign_rY=False, extra_nodes=set(),
        ...
        for var in self.state_vars:
            ...
            if var_name.find('gate') != -1:
                ...
                after_underscore = var_name[var_name.rfind('__')+2:]
                req_section_alpha = 'var_' + var_name[23:len(var_name)-3] + '__alpha_' + after_underscore
                req_section_beta = 'var_' + var_name[23:len(var_name)-3] + '__beta_' + after_underscore
    ...
    def output_bottom_boilerplate(self):
        ...
        for i,var in enumerate(self.state_vars):
            ...
            if init_val is None:
                ...
                init_comm = ''
                #21
            self.writeln('Vmath::Fill(m_nq, ', init_val, ',' ,  ' '*(21-len(init_val)) , 'm_cellSol[', i ,'],  1);',
                       init_comm)
