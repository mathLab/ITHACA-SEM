...
In particular from the \inltt{EXPANSIONS}, \inltt{VARIABLES},
\inltt{BOUNDARYCONDITIONS} and \inltt{FUNCTIONS} sections of the file.
\end{notebox}
...
\subsubsection{Functions:~} The initial conditions and the body force (\textit{i.e.} $f$ in the governing equations \ref{IncNS_equations}) are given by
\begin{lstlisting}[style=XMLStyle]
   <FUNCTION NAME="InitialConditions">
     <E VAR="u" VALUE="1" />
     <E VAR="v" VALUE="0" />
...
\subsubsection{Solver information:~} The flag \inltt{<I PROPERTY="HOMOGENEOUS" VALUE="1D"/>} is informing the code that, even if the mesh is 2D, the problem is 3D and the third direction needs to be added with a Fourier series. The flag \inltt{<I PROPERTY="USEFFT" VALUE="FFTW"/>} activates the FFT routines inside the code. In this case \nekpp uses the FFTW library to move the degrees of freedom from wave to real space. 
...
\subsubsection{Parameters:~} Setting the mean inlet velocity to 1, allows us to define the kinematic viscosity as $\nu = \frac{UD}{Re}=\frac{1}{Re}$. \inltt{HomeModes} and \inltt{LZ} set the number of Fourier modes and the length of the third direction respectively. 
