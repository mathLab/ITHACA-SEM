...
\subsubsection{Substepping the Velocity Correction Scheme}
...
\subsubsubsection{Direct stability analysis}
...
\subsubsubsection{Adjoint Stability Analysis}
...
\subsubsubsection{Transient Growth Analysis}
...
\subsubsection{Steady-state solver using Selective Frequency Damping}
\label{SectionSFD}
...
\subsection{Stability analysis Session file configuration}
\label{SecStabFile}
...
\subsection{Steady-state solver Session file configuration}
...
\subsubsection{Execution of the classical steady-state solver}

\subsubsubsection{Solver Info}
...
\subsubsection{Execution of the adaptive steady-state solver}
...
\subsubsection{Kovasznay Flow 2D}
...
\subsubsubsection{Running the simulation}
...
\subsubsection{Kovasznay Flow 2D using high-order outflow boundary conditions}
...
\subsubsection{Steady Kovasznay Oseen Flow using the direct solver}
...
\subsubsection{Laminar Channel Flow 2D}
...
\subsubsubsection{Running the solver}
\begin{lstlisting}[style=BashInputStyle]
...
\subsubsection{Laminar Channel Flow 3D}
...
\subsubsection{Laminar Channel Flow Quasi-3D}
...
\subsubsection{Turbulent Pipe Flow}
...
\subsubsection{Aortic Blood Flow}
...
\subsubsubsection{Expansion:~} In this example we will use a fourth order polynomial expansion. There are two composites defined here since we have both prismatic and tetrahedral elements.
\begin{lstlisting}[style=XMLStyle]
<EXPANSIONS>
...
\subsubsubsection{Solver information:~}
\begin{lstlisting}[style=XMLStyle]
<SOLVERINFO>
    <I PROPERTY="SolverType" VALUE="VelocityCorrectionScheme" />
    <I PROPERTY="EQTYPE" VALUE="UnsteadyNavierStokes" />
...
\subsubsubsection{Parameters:~} Since we are prescribing a Reynolds number of 300, and to simplify the problem definition, we set the mean inlet velocity to 1, this allows us to define the kinematic viscosity as $\nu = \frac{UD}{Re}=\frac{3.32}{300} = 1/90.36$.
...
\subsubsection{finite-strip modeling of flow past flexible cables}
...
\end{figure}
 ...
 \subsubsection{2D direct stability analysis of the channel flow}
 ...
 \subsubsubsection{Background}
  ...
  \subsubsubsection{Geometry}

   The geometry under consideration is a 2D channel.

   \subsubsubsection{Mesh Definition}
...
\end{figure}
 ...
 \subsubsection{2D adjoint stability analysis of the channel flow}
 ...
 \subsubsubsection{Geometry \& Mesh}
...
\subsubsection{2D Transient Growth analysis of a flow past a backward-facing step}
...
\subsubsubsection{Input Parameters}
...
\subsubsection{BiGlobal Floquet analysis of a of flow past a cylinder}
