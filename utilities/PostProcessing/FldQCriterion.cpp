...
int main(int argc, char *argv[])
{
    ...
    // Calculate Gradient & Vorticity
        ...
        // W_x = Wy - Vz
        Vmath::Vsub(nq,grad[2*nfields+1],1,grad[1*nfields+2],1,outfield1[0],1);
		//W_x^2
		Vmath::Vmul(nq,outfield1[0],1,outfield1[0],1,outfield1[0],1);

		// W_y = Uz - Wx
        Vmath::Vsub(nq,grad[0*nfields+2],1,grad[2*nfields+0],1,outfield2[0],1);
		//W_y^2
		Vmath::Vmul(nq,outfield2[0],1,outfield2[0],1,outfield2[0],1);
	
		// W_z = Vx - Uy
        Vmath::Vsub(nq,grad[1*nfields+0],1,grad[0*nfields+1],1,outfield3[0],1);
		//W_z^2
		...
		Vmath::Vmul(nq,grad[0*nfields+0],1,grad[0*nfields+0],1,outfield1[0],1);
		Vmath::Vmul(nq,grad[1*nfields+1],1,grad[1*nfields+1],1,outfield2[0],1);
		Vmath::Vmul(nq,grad[2*nfields+2],1,grad[2*nfields+2],1,outfield3[0],1);
		...
		//W_y+V_z
		Vmath::Vadd(nq,grad[2*nfields+1],1,grad[1*nfields+2],1,outfield1[0],1);
		Vmath::Vmul(nq,&outfield1[0][0],1,&outfield1[0][0],1,&outfield1[0][0],1);

		//U_z+W_x
		Vmath::Vadd(nq,grad[0*nfields+2],1,grad[2*nfields+0],1,outfield2[0],1);
		Vmath::Vmul(nq,&outfield2[0][0],1,&outfield2[0][0],1,&outfield2[0][0],1);
		
		//V_x+U_y
		Vmath::Vadd(nq,grad[1*nfields+0],1,grad[0*nfields+1],1,outfield3[0],1);
		Vmath::Vmul(nq,&outfield3[0][0],1,&outfield3[0][0],1,&outfield3[0][0],1);

		Vmath::Vadd(nq,&outfield1[0][0],1,&outfield2[0][0],1,&outfield2[0][0],1);
		Vmath::Vadd(nq,&outfield2[0][0],1,&outfield3[0][0],1,&outfield3[0][0],1);
		for(int k=0; k<addfields; ++k)
		{
