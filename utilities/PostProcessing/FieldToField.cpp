...
int main(int argc, char *argv[])
{
    void SetFields(SpatialDomains::MeshGraphSharedPtr &mesh,
                   ...
                   const vector<std::string> &variables, bool homogeneous);
                      ...
                      Array<OneD, NekDouble> z)
{
    ...
    ASSERTL0(field0.num_elements() == field1.num_elements(),"Input field dimension must be same as output dimension");
    ...
    for(r=0; r< nq1; r++)
    {
        ...
        {
            NekDouble value;
            value = field0[f]->GetExp(elmtid)->StdPhysEvaluate(Lcoords, field0[f]->GetPhys() +offset);    
                ...
                Array<OneD, MultiRegions::ExpListSharedPtr> &outfield)
{
    ...
    {  
        ...
        {
            //plane 0
            outfield[j]->GetPlane(0)->FwdTrans_IterPerExp(outfield[j]->GetPlane(0)->GetPhys(),
                                                          outfield[j]->GetPlane(0)->UpdateCoeffs());
            
            //plane 1
            outfield[j]->GetPlane(1)->FwdTrans_IterPerExp(outfield[j]->GetPlane(1)->GetPhys(),
