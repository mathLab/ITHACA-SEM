	...
	//~ {
	    //~ fprintf(stderr,
		    //~ "Usage: ./Gentien meshfile fieldFile Nprof L Npoint idebut ifin \n");
	    //~ fprintf(stderr,
	...
	cout << "Lecture et calcul annexe en cours ... veuillez patientez" << endl;
...
// 			Vmath::Vdiv(nbc,lift_v,1,CoeffAero,1, lift_v,1);
	...
	// Before we have to reorganize the array because there is some double and the data are not classified.
	...
	int		  compteur=0;
	NekDouble Coeff;
	NekDouble maxX=surfaceX[0];
	NekDouble minX=surfaceX[0];
	NekDouble maxY=surfaceY[0];
	NekDouble minY=surfaceY[0];
	
	ofstream coord;
    coord.open("coordonnee.dat");
		...
		for (int i=0; i<id1; i++)
	{
		...
		{
			if ((surfaceX[j]==surfaceX[i]) && (surfaceY[j]==surfaceY[i]))  
			{
				for (int k=j ; k<id1-1; k++)
				{
				surfaceX[k]=surfaceX[k+1];
				surfaceY[k]=surfaceY[k+1];
				}	
				id1=id1-1;
			}
		
			if (surfaceX[j] < surfaceX[i] )
			{
			inter=surfaceX[i];
			inter2=surfaceY[i];
			surfaceX[i]=surfaceX[j];
			surfaceY[i]=surfaceY[j];
			surfaceX[j]=inter;
			surfaceY[j]=inter2;
			}		
		}
				...
				if (surfaceY[int(id1/2)-5+i] > surfaceY[id1])
				{
					compteur=compteur+1;
				}
				}
		...
		// Now start the extraction of the data
	
	  NekDouble Nprof = atoi(argv[3]);
	  NekDouble Npoint =atoi(argv[5]);
      NekDouble x0=  0.0;
      NekDouble y0=  0.0;
      NekDouble z0=  0.0;
	  NekDouble dy=  0.0;
      ...
      Array<OneD, NekDouble> pointX((id1)*pg-1);
      Array<OneD, NekDouble> pointY((id1)*pg-1);
	...
	if (cas==1) 
	{
		...
		dy=atof(argv[4])/(Npoint>1 ? (Npoint-1):1);
		
		for (int i=0; i< id1 ; i++)
		{		
			cout << "avancement en cours" << "   " << i << "/" << id1 << endl;
	
			
			
			
			if (surfaceX[i+1]==surfaceX[i]) 
				{
				dx=0.0;
				dy=0.0;
				}
				else if (surfaceY[i+1]==surfaceY[i]) 
				{
				dx=0.0;
				dy=atof(argv[4])/(Npoint>1 ? (Npoint-1):1);
				}
				...
				{
				Coeff=(surfaceY[i+1]-surfaceY[i])/(surfaceX[i+1]-surfaceX[i]);	
	
				dx=atof(argv[4])/(Npoint>1 ? (Npoint-1):1)*sqrt(Coeff*Coeff/(Coeff*Coeff+1));
				dy=-dx/Coeff;
				}		
				...
				for (int j=0; j<pg; j++)
				{
						...
						{
						x0=surfaceX[i]+((surfaceX[i+1]-surfaceX[i])/(pg-1))*j;
						y0=surfaceY[i]+((surfaceY[i+1]-surfaceY[i])/(pg-1))*j;
						}
				...
				for (int k = 0; k < Npoint ; k++)
					...
					coord << gloCoord[0] << "    " << gloCoord[1] << endl;	
						}			
				}			
		}
	}
	...
	{		
		...
		for (int i=0; i<id1; i++)
	{
			surfaceX[i]=surfaceX[i]-maxX;
			surfaceY[i]=surfaceY[i]-maxY;
		
		
			if ((surfaceX[i]>0) && (surfaceY[i]>0))
			{
				surfaceZ[i]=atan(surfaceY[i]/surfaceX[i]);
			}
			else if((surfaceX[i]<0) && (surfaceY[i]>0))
			{
				surfaceZ[i]=atan(surfaceY[i]/surfaceX[i])+3.141592;
			}
			else if((surfaceX[i]<0) && (surfaceY[i]<0))
			{
				surfaceZ[i]=atan(surfaceY[i]/surfaceX[i])+3.141592;
			...
			{
				surfaceZ[i]=atan(surfaceY[i]/surfaceX[i])+6.28318;
			}	
			else if((surfaceX[i]==0))
			{
				if (surfaceY[i] > 0)
				{
				surfaceZ[i]=1.5709;
				}
				else if (surfaceY[i] < 0)
				{
				surfaceZ[i]=4.71238;
				}
			}	
	...
	for (int i=0; i<id1; i++)
	{
		...
		NekDouble intervalle=0.0;
		int debut=int(atof(argv[6]));
		int fin=int(atof(argv[7]));
		...
		{    
		  cout << "Avancement en cours " << "   " << i << "   " << "sur " << "   " << id1  << endl;
		  ...
		  surfaceY[i]=surfaceY[i]+maxY;
			...
			if (i==id1-1)
			{
			dx=atof(argv[4])/(Npoint>1 ? (Npoint-1):1);
			}
			else
			{
			if (surfaceX[i]>0 || ((surfaceX[i]==0)&&(surfaceZ[i]<3)))
			{
			...
			else if (surfaceX[i]<0|| ((surfaceX[i]==0)&&(surfaceZ[i]>3)))
			{
				if (surfaceX[i+1]==surfaceX[i]) 
				{
				dx=0.0;
				...
				for (int k = 0; k < Npoint; k++)
					{			
						...
						file << gloCoord[0] << "   " << gloCoord[1] << endl;
 ...
 NekDouble dxx=(surfaceX[fin]-surfaceX[debut])/(Nprof-1);
 NekDouble dyy=(surfaceY[fin]-surfaceY[debut])/(Nprof-1);
  
 
	for (int i=0; i<Nprof; i++)
	{
		
 		if (surfaceX[debut]>0 || ((surfaceX[debut]==0)&&(surfaceZ[debut]<3)))
			{
			...
			else if (surfaceX[debut]<0|| ((surfaceX[debut]==0)&&(surfaceZ[debut]>3)))
			{
				if (dxx==0) 
				{
				dx=0.0;
			...
			x0=surfaceX[debut]+i*dxx	;
			y0=surfaceY[debut]+i*dyy	;
     ...
     * for (int j=0; j<=pg-1; j++)
				...
				{
				pointX[i*(pg)+j]=surfaceX[i]+((surfaceX[i+1]-surfaceX[i])/(pg+1))*j;
				pointY[i*(pg)+j]=surfaceY[i]+((surfaceY[i+1]-surfaceY[i])/(pg+1))*j;;
				
				cout << pointX[i*(pg)+j] << pointY[i*(pg)+j] << endl;
...
/***
Ici on va essayer de traiter le cas avec la bosse: il faut faire un programme qui extrait les donner sans prendre en compte la bosse, 
* il faut extraire les données comme si on avait pas de bosse, pour ce faire on va 
* 1°) Cherche le début et la fin de la bosse (au début donnée par l'utilisateur)
* 2°) Localiser les élements de la numérotation (i=0; i< id1) coorespondant au début et à la fin  , ie celui le plus proche pour qu'on ait
* surfaceX[element]=Xdebut
* surfaceY[element]=Ydebut
* Et idem pour la fin
* 3°) Tracer une droite entre le point du début et le point de la fin
* 4°) Calculer la normal à cette droite
* 5°) Prendre un nombre fixé de point et extraire les normal le long de cette droite au nombre de point voulus
* 
* Argument d'entrée:
* meshfile fieldfile Npoint L Nprof Nbosse (qu'on prend = à Nprof) xdébut ydebut xfin yfin
* 
* 
* NekDouble intervalle=0.0;
* int debut=0;
...
* {
* 	
* intervalle= (0.1/100)*k
* 
* for (int i=0; i< id1; i++)
* {  
* 
* if ((-intervalle<(surfaceX[i]-xdébut)) && (intervalle>(surfaceX[i]-xdébut)) && (-intervalle<(surfaceY[i]-ydébut)) && (intervalle>(surfaceY[i]-ydébut)))
* 
* debut=i;
* 
* break;
...
* {  
* 
* if ((-intervalle<(surfaceX[i]-xfin)) && (intervalle>(surfaceX[i]-xfin)) && (-intervalle<(surfaceY[i]-yfin)) && (intervalle>(surfaceY[i]-yfin)))
* 
* fin=i;
* 
* break;
...
* {
* on extrait en mode normal ...
* }
* 
* dxx=surfaceX[debut]+(surfaceX[fin]-surfaceX[debut])/(Nprof-1);
* dyy=surfaceY[debut]+(surfaceY[fin]-surfaceY[debut])/(Nprof-1);
* 
* 
* 
* for (int i=0; i<Nprof; i++)
