...
int regDemo::Test(std::string errStr){
	...
	for(i=0;i<regDemo::numTest;++i){
		...
		if(modes!=regDemo::Modes[i]){
			cout << regDemo::Geometry[i] << " -> " << modes << "::"<<regDemo::Modes[i]<<endl;
			fail=5;
			continue;
		}
...
int regDemo::makeRegress(){
	...
	for (int i=0;i<regDemo::numTest;++i){
		...
		if (!out) {
			// std::cout << "Could not open output file (" << regDemo::Output << ")!" << std::endl;
			return 3;
		}
...
int regDemo::readRegress(std::string mesh, std::string errStr, std::string &L2e,int &modes){
	FILE *mRes;
	char buffer[50];
