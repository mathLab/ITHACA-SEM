...
\begin{notebox}
  The default is to know output .xml files in a commpressed form where the VERTEX, EDGES, FACES, ELEMENTS and CURVED information is compressed into binrary format which is then converted into base64. This is identified by the attribute \inltt{COMPRESSED="B64Z''}. 
  To output in ascii format you need to add the extension ``:xml:uncompress'' to the .xml file, i.e. \\
  \inltt{ MeshConvert file.msh file.xml:xml:uncompress}
\end{notebox}
