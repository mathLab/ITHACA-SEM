/**
 * This function calculate the divergence starting from an .fld file.
 * It is meant to be used with solutions produced by the
 * incompressible Navier-Stokes solver.  To use it with solutions
 * coming form another solver further generalisations are required.
 */
#include <cstdio>
#include <cstdlib>
...
int main(int argc, char *argv[])
{
    ...
    if(argc != 3)
    {
        fprintf(stderr,"Usage: ./CalcDivergence file.xml file.fld\n");
        exit(1);
    }
    ...
    Array<OneD, MultiRegions::ExpListSharedPtr> Exp(nfields + 1);
    ...
    Array<OneD, NekDouble> Ux(nq);
    Array<OneD, NekDouble> Vy(nq);
    Array<OneD, NekDouble> Div(nq);  
    ...
    case 2:
        {
            ...
            {
                Exp[0]->PhysDeriv(MultiRegions::DirCartesianMap[0],Exp[0]->GetPhys(),Ux);
                Exp[0]->PhysDeriv(MultiRegions::DirCartesianMap[1],Exp[1]->GetPhys(),Vy);
		
                Vmath::Vsub(nq,Ux,1,Vy,1,Div,1);
		
                Exp[3]->FwdTrans(Div,Exp[3]->UpdateCoeffs());
            }
        }
        break;
    case 3:
    ...
    string   endfile("_with_divergence.fld");
    out += endfile;
    ...
    for(j = 0; j < nfields + 1; ++j)
    {
        for(i = 0; i < FieldDef.size(); ++i)
        {
            if (j >= nfields)
            {
                FieldDef[i]->m_fields.push_back("Div");
            }
            else
            {
